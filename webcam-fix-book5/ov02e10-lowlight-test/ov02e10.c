 ...
 * On ACPI/IPU7 platforms the IPU bridge registers a fixed clock,
...
/*
 * EXPERIMENTAL — issue #67 (vertical lines / column FPN in low light).
 *
 * The stripes are column fixed-pattern noise in the sensor's analog readout,
 * and they scale with analog gain. In a dim room AGC pegs at the sensor's
 * maximum gain (15.5x) because it has already run out of exposure time, so the
 * FPN gets amplified fifteen-fold.
 *
 * Exposure is capped at vts_def - 2 = 2242 lines. Nothing in userspace can lift
 * that: libcamera's simple pipeline never touches V4L2_CID_VBLANK and has no
 * frame-duration control, the soft IPA caches exposureMax at configure() time,
 * and this driver resets VBLANK to the mode default on every set_fmt — which is
 * why manually setting VBLANK with v4l2-ctl gets wiped the moment a camera opens.
 *
 * So the only lever is the mode's VTS. Raising it lengthens the frame, which
 * raises the exposure ceiling, which should let AGC back off the gain — trading
 * frame rate for a cleaner picture.
 *
 * Default 0 = use the mode's value (2244), i.e. stock behaviour, unchanged.
 *
 *   modprobe ov02e10 vts=4488     # ~half the frame rate, ~2x the exposure ceiling
 */
static int vts;
module_param(vts, int, 0644);
MODULE_PARM_DESC(vts,
	"Override the mode's VTS (frame height in lines) to raise the exposure ceiling and let AGC use less gain. 0 = stock (default). Experimental, see issue #67.");
...
/* Effective VTS: the `vts` module param if sane, else the mode's own. (#67) */
static u32 ov02e10_vts(const struct ov02e10_mode *mode)
{
	if (vts >= (int)mode->vts_min && vts <= OV02E10_VTS_MAX)
		return vts;
	return mode->vts_def;
}
...
static int ov02e10_init_controls(struct ov02e10 *ov02e10)
{
	...
	vblank_def = ov02e10_vts(mode) - mode->height;
